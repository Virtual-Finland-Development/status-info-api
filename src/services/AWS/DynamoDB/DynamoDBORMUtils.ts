...
import { getDynamoDBModel, ModelName } from "../../../data/DataManager";
import { ValidationError } from "../../../utils/exceptions";
import Settings from "../../../utils/Settings";
import { cloneItem } from "../../../utils/Transformations";
import { DDBPrimitive, DDBSearchClause, DynamoDBModel, KnownAutoGeneratedTypes, LooseDynamoDBRecord, PrimitiveDynamoDBRecord } from "./DynamoDBORMTypes";
...
export async function getSchema(tableName: ModelName): Promise<DynamoDBModel["schema"]> {
  return getDynamoDBModel(tableName).schema;
}
...
function mutateDynamoDBAutoFills(schema: DynamoDBModel["schema"], item: PrimitiveDynamoDBRecord, event: "create" | "update"): void {
  ...
  for (const autoGenAttr of currentEventAutoFillAttributes) {
    ...
    switch (_AutoGenerated?.autoGenType) {
      case KnownAutoGeneratedTypes.uuidv5:
        item[AttributeName] = generateUUIDv5(item, _AutoGenerated?.autoGenFields);
        break;
      case KnownAutoGeneratedTypes.uuidv4:
        item[AttributeName] = uuidv4();
        break;
      case KnownAutoGeneratedTypes.timestamp:
...
function validateDynamoDBValueUpdate(schema: DynamoDBModel["schema"], item: PrimitiveDynamoDBRecord) {
  ...
  for (const needyAttribute of needyAttributes) {
    ...
    if (typeof _AllowedValues !== "undefined" && !_AllowedValues.includes(itemValue)) {
...
export async function resolveQueryableSearch(tableName: ModelName, searchClause: DDBSearchClause): Promise<DDBSearchClause> {
  ...
  for (const keySchemaKey of keySchemaKeys) {
    ...
    if (keySchemaKeyDefinition?._AutoGenerated?.autoGenType === KnownAutoGeneratedTypes.uuidv5) {
